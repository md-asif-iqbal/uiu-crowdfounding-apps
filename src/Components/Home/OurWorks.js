...
const OurWorks = () => {
  ...
  useEffect(() => {
      fetch('http://localhost:8000/ourwroks')
          .then(res => res.json())
    ...
    return (
        <div className='my-10 '>
            <h1 className='text-2xl font-bold underline text-rose-700 mb-8'>Our Works</h1>
  ...
  <div class="p-4 bg-gray-900">
    ...
    {
      item.loan? <button type="" className='btn mt-2 bg-green-500'> {
          user? <Link to="/requestforloan">{item.loan}</Link> :
          <Link to="/login">{item.loan}</Link> 
      }</button>
      : <button type="" className='btn mt-2 bg-green-500'> {
        user? <Link to="/donate">{item.donate}</Link>
