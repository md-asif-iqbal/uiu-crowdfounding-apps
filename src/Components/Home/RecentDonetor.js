...
const RecentDonetor = () => {
  ...
  useEffect(() => {
    fetch("https://uiu-crowdfounding-apps.herokuapp.com/recentDonator")
      .then((res) => res.json())
      .then((data) => setDonates(data));
  }, []);
  return (
    <div>
      <h1 className="text-2xl font-bold underline text-rose-700 mt-10 mb-8">
        Recent Donar
      </h1>
        ...
        {donates.map((item) => (
          <div class="relative block p-8 overflow-hidden border border-gray-100 rounded-lg">
            ...
            <div class="justify-between sm:flex">
              <div>
                <h5 class="text-xl font-bold text-gray-900">{item.details}</h5>
